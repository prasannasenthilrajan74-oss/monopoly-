...
export class GameManager {
    ...
    private dice: [number, number] = [0, 0];
    private gameStatus: 'waiting' | 'active' | 'finished' = 'waiting';
    private propertyOwners: Record<number, string> = {};
    private buildings: Record<number, number> = {};
    private currentTurnPhase: 'rolling' | 'acting' | 'jail_decision' = 'rolling';
    private lastAction: string = 'Game started';
    private doublesConsecutive: number = 0;
    ...
    private availableColors: string[] = ['#f44336', '#2196f3', '#4caf50', '#ffeb3b', '#9c27b0', '#ff9800', '#00bcd4', '#e91e63'];
    ...
    public rollDice(playerId: string): { dice: [number, number], player: Player | undefined } {
        const player = this.players.find(p => p.id === playerId);
        if (!player) return { dice: [0, 0], player: undefined };

        if (this.players[this.currentPlayerIndex].id !== playerId) return { dice: this.dice, player };
        if (this.currentTurnPhase !== 'rolling' && this.currentTurnPhase !== 'jail_decision') return { dice: this.dice, player };
        ...
        if ((space.type === 'property' || space.type === 'railroad' || space.type === 'utility') && ownerId && ownerId !== player.id) {
            ...
            if (owner && space.price) {
                ...
                if (space.type === 'railroad') {
                    ...
                    const rrCount = BOARD.filter(s => s.type === 'railroad' && this.propertyOwners[s.id] === owner.id).length;
                    rent = [25, 50, 100, 200][rrCount - 1] || 25;
                } else if (space.type === 'utility') {
                    // Dice dependent
                    const uCount = BOARD.filter(s => s.type === 'utility' && this.propertyOwners[s.id] === owner.id).length;
    ...
    public buyProperty(playerId: string, propertyId: number) {
        const player = this.players.find(p => p.id === playerId);
        if (!player || this.players[this.currentPlayerIndex].id !== playerId || this.currentTurnPhase !== 'acting') return;
        if (this.propertyOwners[propertyId]) return;
    ...
    public buyHouse(playerId: string, propertyId: number) {
        const player = this.players.find(p => p.id === playerId);
        if (!player || this.players[this.currentPlayerIndex].id !== playerId || this.currentTurnPhase !== 'acting') return;

        const space = BOARD.find(s => s.id === propertyId);
        if (!space || !space.houseCost || space.type !== 'property') return;

        // Check ownership
        if (this.propertyOwners[propertyId] !== playerId) return;
    ...
    public payJailFine(playerId: string) {
        const player = this.players.find(p => p.id === playerId);
    ...
    public endTurn(playerId: string, force: boolean = false) {
        if (!force) {
            if (this.players[this.currentPlayerIndex].id !== playerId) return;
            if (this.currentTurnPhase !== 'acting') return;
            // If double rolled and not in jail, can roll again?
            const isDouble = this.dice[0] === this.dice[1] && this.dice[0] !== 0;
            const player = this.players.find(p => p.id === playerId);

            if (isDouble && !player?.inJail && this.doublesConsecutive < 3) {
                this.currentTurnPhase = 'rolling';
                this.lastAction = `${player?.name} rolled doubles! Roll again.`;
                this.clearTimer();
                return;
            }
        }
